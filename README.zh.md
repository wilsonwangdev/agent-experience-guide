...
1. [什么是智能体体验？](docs/00-what-is-ax.md) — 为什么 AX 重要，UX → DX → AX 演进

### 六个阶段

2. [可发现性](docs/01-discovery.md) — 智能体能找到你吗？llms.txt、robots.txt、站点地图
3. [身份标识](docs/02-identity.md) — 智能体能理解你的产品吗？机器可读描述、元数据
4. [认证与访问](docs/03-auth-and-access.md) — 智能体能认证并执行操作吗？OAuth、auth.md、作用域令牌
5. [集成](docs/04-integration.md) — 管道就绪了吗？MCP、A2A、流式、SDK
6. [错误与恢复](docs/05-errors-and-recovery.md) — 智能体能自愈吗？类型化错误、重试指引

### 架构与实践

7. [智能体原生架构](docs/06-agent-native-architecture.md) — 共享动作模型、智能体 UI 对等
8. [终端用户体验](docs/06b-end-user-experience.md) — 交接流程、MCP Apps
9. [反模式](docs/07-anti-patterns.md) — 25 个破坏智能体体验的常见错误
10. [就绪度检查清单](docs/08-checklist.md) — 实践评估

### 参考

- [参考资料](docs/references.md) — 精选链接和标准
